...
public class PolarizerRecipeBuilder implements RecipeBuilder
{
    ...
    {
        recipeId = new ResourceLocation(recipeId.getNamespace(), recipeId.getPath() + "_using_polarizer");
        Advancement.Builder builder = output.advancement()
                .addCriterion("has_the_recipe", RecipeUnlockedTrigger.unlocked(recipeId))
                .rewards(AdvancementRewards.Builder.recipe(recipeId))
                .requirements(AdvancementRequirements.Strategy.OR);
        criteria.forEach(builder::addCriterion);
        PolarizerRecipe recipe = new PolarizerRecipe(input, new ItemStack(this.output, count), count, processingTime, energyUsage);
