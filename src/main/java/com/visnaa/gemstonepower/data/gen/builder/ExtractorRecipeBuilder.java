...
public class ExtractorRecipeBuilder implements RecipeBuilder
{
    ...
    {
        recipeId = new ResourceLocation(recipeId.getNamespace(), recipeId.getPath() + "_using_extractor");
        Advancement.Builder builder = output.advancement()
                .addCriterion("has_the_recipe", RecipeUnlockedTrigger.unlocked(recipeId))
                .rewards(AdvancementRewards.Builder.recipe(recipeId))
                .requirements(AdvancementRequirements.Strategy.OR);
        criteria.forEach(builder::addCriterion);
        ExtractorRecipe recipe = new ExtractorRecipe(input, new ItemStack(this.output, count), count, processingTime, energyUsage);
