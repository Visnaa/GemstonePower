...
public class FissionReactorBE extends MachineBE<FissionReactorRecipe>
{
    ...
    public void tick(Level level, BlockPos pos, BlockState state)
    {
        ...
        if (changed)
        {
            ...
            PacketDistributor.ALL.noArg().send(new FissionReactorSyncS2C(activated, getProcessingProgress(), getProcessingTotalTime(), energyGeneration, heatGeneration, heat, maxHeat, pos));
        }
    }
    ...
    public void setFromClientPacket(boolean activated)
    {
        this.activated = activated;
        setChanged();
        PacketDistributor.ALL.noArg().send(new FissionReactorSyncS2C(activated, getProcessingProgress(), getProcessingTotalTime(), energyGeneration, heatGeneration, heat, maxHeat, getBlockPos()));
    }
