...
{
    public static final Codec<CrystalConfiguration> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                BuiltInRegistries.BLOCK.byNameCodec().fieldOf("block").flatXmap(CrystalConfiguration::apply, DataResult::success).orElse((CrystalBlock) ModBlocks.RUBY_CRYSTALS.get()).forGetter(getter -> getter.placeBlock),
                Codec.intRange(1, 64).fieldOf("search_range").orElse(10).forGetter(getter -> getter.searchRange),
                Codec.BOOL.fieldOf("can_place_on_floor").orElse(false).forGetter(getter -> getter.canPlaceOnFloor),
                Codec.BOOL.fieldOf("can_place_on_ceiling").orElse(false).forGetter(getter -> getter.canPlaceOnCeiling),
                Codec.BOOL.fieldOf("can_place_on_wall").orElse(false).forGetter(getter -> getter.canPlaceOnWall),
                Codec.floatRange(0.0F, 1.0F).fieldOf("chance_of_spreading").orElse(0.5F).forGetter(getter -> getter.chanceOfSpreading),
                RegistryCodecs.homogeneousList(Registries.BLOCK).fieldOf("can_be_placed_on").forGetter(getter -> getter.canBePlacedOn))
    ...
    public List<Direction> getShuffledDirectionsExcept(RandomSource random, Direction direction) {
        return Util.toShuffledList(this.validDirections.stream().filter(dir -> dir != direction), random);
    }
