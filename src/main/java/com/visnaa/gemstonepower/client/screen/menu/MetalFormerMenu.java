...
public class MetalFormerMenu extends AbstractContainerMenu
{
    ...
    public MetalFormerMenu(int id, Inventory inventory)
    {
        this(ModContainers.METAL_FORMER.get(), ModRecipes.METAL_FORMER_RECIPE, id, inventory, new SimpleContainer(3), new SimpleContainerData(4));
    }

    public MetalFormerMenu(int id, Inventory inventory, Container container, ContainerData data)
    {
        this(ModContainers.METAL_FORMER.get(), ModRecipes.METAL_FORMER_RECIPE, id, inventory, container, data);
    }

    protected MetalFormerMenu(MenuType<?> menu, RecipeType<? extends MetalFormerRecipe> recipe, int id, Inventory inventory, Container container, ContainerData data)
    {
        super(menu, id);
        ...
        this.addSlot(new Slot(container, 1, 16, 38));
    ...
    protected boolean canProcess(ItemStack itemStack, ItemStack preset)
    {
        return this.level.getRecipeManager().getRecipeFor((RecipeType<MetalFormerRecipe>) this.recipeType, new SimpleContainer(itemStack, preset), this.level).isPresent();
    }
