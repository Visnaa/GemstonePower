...
public class CrystalGrowerMenu extends AbstractContainerMenu
{
    ...
    private final RecipeType<? extends CrystalGrowerRecipe> recipeType;

    public CrystalGrowerMenu(int id, Inventory inventory)
    {
        this(ModContainers.CRYSTAL_GROWER.get(), ModRecipes.CRYSTAL_GROWER_RECIPE, id, inventory, new SimpleContainer(2), new SimpleContainerData(4));
    }

    public CrystalGrowerMenu(int id, Inventory inventory, Container container, ContainerData data)
    {
        this(ModContainers.CRYSTAL_GROWER.get(), ModRecipes.CRYSTAL_GROWER_RECIPE, id, inventory, container, data);
    }

    protected CrystalGrowerMenu(MenuType<?> menu, RecipeType<? extends CrystalGrowerRecipe> recipe, int id, Inventory inventory, Container container, ContainerData data)
    {
        super(menu, id);
    ...
    {
        return this.level.getRecipeManager().getRecipeFor((RecipeType<CrystalGrowerRecipe>) this.recipeType, new SimpleContainer(itemStack), this.level).isPresent();
    }

    public int getGrowingProcess()
    {
