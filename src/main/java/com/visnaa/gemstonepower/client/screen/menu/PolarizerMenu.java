...
public class PolarizerMenu extends AbstractContainerMenu
{
    private final Container container;
    private final ContainerData data;
    protected final Level level;
    private final RecipeType<? extends PolarizerRecipe> recipeType;

    public PolarizerMenu(int id, Inventory inventory)
    {
        this(ModContainers.POLARIZER.get(), ModRecipes.POLARIZER_RECIPE, id, inventory, new SimpleContainer(2), new SimpleContainerData(4));
    }

    public PolarizerMenu(int id, Inventory inventory, Container container, ContainerData data)
    {
        this(ModContainers.POLARIZER.get(), ModRecipes.POLARIZER_RECIPE, id, inventory, container, data);
    }

    protected PolarizerMenu(MenuType<?> menu, RecipeType<? extends PolarizerRecipe> recipe, int id, Inventory inventory, Container container, ContainerData data)
    {
        super(menu, id);
    ...
    {
        return this.level.getRecipeManager().getRecipeFor((RecipeType<PolarizerRecipe>) this.recipeType, new SimpleContainer(itemStack), this.level).isPresent();
    }

    public int getPolarizingProgress()
    {
