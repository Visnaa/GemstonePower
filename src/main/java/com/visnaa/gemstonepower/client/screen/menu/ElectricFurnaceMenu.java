...
public class ElectricFurnaceMenu extends AbstractContainerMenu
{
    ...
    {
        this(ModContainers.ELECTRIC_FURNACE.get(), RecipeType.SMELTING, id, inventory, new SimpleContainer(2), new SimpleContainerData(4));
    }

    public ElectricFurnaceMenu(int id, Inventory inventory, Container container, ContainerData data)
    {
        this(ModContainers.ELECTRIC_FURNACE.get(), RecipeType.SMELTING, id, inventory, container, data);
    }

    protected ElectricFurnaceMenu(MenuType<?> menu, RecipeType<? extends SmeltingRecipe> recipe, int id, Inventory inventory, Container container, ContainerData data)
    {
        super(menu, id);
    ...
    {
        return this.level.getRecipeManager().getRecipeFor((RecipeType<SmeltingRecipe>) this.recipeType, new SimpleContainer(itemStack), this.level).isPresent();
    }

    public int getCookingProcess()
    {
        int i = this.data.get(0);
        int j = this.data.get(1);
        return j != 0 && i != 0 ? (i * 22 / j) + 2 : 0;
    }
