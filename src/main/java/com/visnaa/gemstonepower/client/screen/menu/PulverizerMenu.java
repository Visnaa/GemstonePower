...
public class PulverizerMenu extends AbstractContainerMenu
{
    ...
    private final RecipeType<? extends PulverizerRecipe> recipeType;

    public PulverizerMenu(int id, Inventory inventory)
    {
        this(ModContainers.PULVERIZER.get(), ModRecipes.PULVERIZER_RECIPE, id, inventory, new SimpleContainer(2), new SimpleContainerData(4));
    }

    public PulverizerMenu(int id, Inventory inventory, Container container, ContainerData data)
    {
        this(ModContainers.PULVERIZER.get(), ModRecipes.PULVERIZER_RECIPE, id, inventory, container, data);
    }

    protected PulverizerMenu(MenuType<?> menu, RecipeType<? extends PulverizerRecipe> recipe, int id, Inventory inventory, Container container, ContainerData data)
    {
        super(menu, id);
    ...
    {
        return this.level.getRecipeManager().getRecipeFor((RecipeType<PulverizerRecipe>) this.recipeType, new SimpleContainer(itemStack), this.level).isPresent();
    }
