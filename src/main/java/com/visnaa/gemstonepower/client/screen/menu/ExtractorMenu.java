...
public class ExtractorMenu extends AbstractContainerMenu
{
    ...
    private final RecipeType<? extends ExtractorRecipe> recipeType;

    public ExtractorMenu(int id, Inventory inventory)
    {
        this(ModContainers.EXTRACTOR.get(), ModRecipes.EXTRACTOR_RECIPE, id, inventory, new SimpleContainer(2), new SimpleContainerData(4));
    }

    public ExtractorMenu(int id, Inventory inventory, Container container, ContainerData data)
    {
        this(ModContainers.EXTRACTOR.get(), ModRecipes.EXTRACTOR_RECIPE, id, inventory, container, data);
    }

    protected ExtractorMenu(MenuType<?> menu, RecipeType<? extends ExtractorRecipe> recipe, int id, Inventory inventory, Container container, ContainerData data)
    {
        super(menu, id);
    ...
    {
        return this.level.getRecipeManager().getRecipeFor((RecipeType<ExtractorRecipe>) this.recipeType, new SimpleContainer(itemStack), this.level).isPresent();
    }
