...
public class SawmillMenu extends AbstractContainerMenu
{
    ...
    private final RecipeType<? extends SawmillRecipe> recipeType;

    public SawmillMenu(int id, Inventory inventory)
    {
        this(ModContainers.SAWMILL.get(), ModRecipes.SAWMILL_RECIPE, id, inventory, new SimpleContainer(2), new SimpleContainerData(4));
    }

    public SawmillMenu(int id, Inventory inventory, Container container, ContainerData data)
    {
        this(ModContainers.SAWMILL.get(), ModRecipes.SAWMILL_RECIPE, id, inventory, container, data);
    }

    protected SawmillMenu(MenuType<?> menu, RecipeType<? extends SawmillRecipe> recipe, int id, Inventory inventory, Container container, ContainerData data)
    {
        super(menu, id);
    ...
    {
        return this.level.getRecipeManager().getRecipeFor((RecipeType<SawmillRecipe>) this.recipeType, new SimpleContainer(itemStack), this.level).isPresent();
    }

    public int getSawingProgress()
    {
